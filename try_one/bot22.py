...
# Store recent messages (to detect context over time)
...
# Function to check for drug-related keywords
...
# Function to detect patterns of quantities or payments
...
# Sentiment Analysis to detect urgency or aggression
...
# Contextual Analysis: Monitor chats over time
...
# Function to analyze full chat history
...
# Telegram event handler to start chat history analysis
@client.on(events.NewMessage(pattern='/analyze'))
async def start_analysis(event):
    chat_id = event.chat_id
    await event.respond("Starting analysis of chat history... Please wait.")
    await analyze_full_chat_history(chat_id)
    await event.respond("Analysis completed.")
