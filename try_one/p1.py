...
# 2. Common slang/codes for drugs (can be extended)
DRUG_CODES = [
...
def detect_drug_trafficking(message):
    ...
    # 1. Check for drug-related keywords
    ...
    if is_aggressive_message(message):
        return True, "Aggressive or urgent tone detected"
...
async def message_handler(event):
    ...
    # Log or respond in the same chat to notify users (optional)
    await event.reply(f"Message analyzed: {reason}")
